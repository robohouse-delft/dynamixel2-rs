	...
	WriteBuffer: AsRef<[u8]> + AsMut<[u8]>,
{
	...
	pub fn sync_read<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16, count: u16) -> Result<Vec<SyncData<Vec<u8>>>, TransferError> {
	...
	pub fn sync_read_u8<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16) -> Result<Vec<SyncData<u8>>, TransferError> {
		let mut result = Vec::with_capacity(motor_ids.len());
		let mut read_error = None;
	...
	pub fn sync_read_u16<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16) -> Result<Vec<SyncData<u16>>, TransferError> {
		let mut result = Vec::with_capacity(motor_ids.len());
		let mut read_error = None;
	...
	pub fn sync_read_u32<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16) -> Result<Vec<SyncData<u32>>, TransferError> {
		let mut result = Vec::with_capacity(motor_ids.len());
		let mut read_error = None;
	...
	WriteBuffer: AsRef<[u8]> + AsMut<[u8]>,
{
	...
	/// The `on_response` function is called for the reply from each motor.
	/// If the function fails to write the instruction, an error is returned and the function is not called.
	pub async fn sync_read_u8_cb<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16, mut on_response: F) -> Result<(), WriteError>
	where
		F: FnMut(Result<SyncData<u8>, ReadError>),
	{
		let count = 1;
	...
	/// The `on_response` function is called for the reply from each motor.
	/// If the function fails to write the instruction, an error is returned and the function is not called.
	pub async fn sync_read_u16_cb<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16, mut on_response: F) -> Result<(), WriteError>
	where
		F: FnMut(Result<SyncData<u16>, ReadError>),
	{
		let count = 1;
	...
	/// The `on_response` function is called for the reply from each motor.
	/// If the function fails to write the instruction, an error is returned and the function is not called.
	pub async fn sync_read_u32_cb<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16, mut on_response: F) -> Result<(), WriteError>
	where
		F: FnMut(Result<SyncData<u32>, ReadError>),
	{
		let count = 4;
	...
	/// If this function fails to get the data from any of the motors, the entire function retrns an error.
	/// If you need access to the data from other motors, or if you want acces to the error for each motor, see [`Self::sync_read_u8_cb`].
	pub async fn sync_read_u8<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16) -> Result<Vec<SyncData<u8>>, TransferError> {
		let mut result = Vec::with_capacity(motor_ids.len());
		let mut read_error = None;
		self.sync_read_u8_cb(motor_ids, address, |data| match data {
			Err(e) if read_error.is_none() => read_error = Some(e),
	...
	/// If this function fails to get the data from any of the motors, the entire function retrns an error.
	/// If you need access to the data from other motors, or if you want acces to the error for each motor, see [`Self::sync_read_u16_cb`].
	pub async fn sync_read_u16<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16) -> Result<Vec<SyncData<u16>>, TransferError> {
		let mut result = Vec::with_capacity(motor_ids.len());
		let mut read_error = None;
		self.sync_read_u16_cb(motor_ids, address, |data| match data {
			Err(e) if read_error.is_none() => read_error = Some(e),
	...
	/// If this function fails to get the data from any of the motors, the entire function retrns an error.
	/// If you need access to the data from other motors, or if you want acces to the error for each motor, see [`Self::sync_read_u32_cb`].
	pub async fn sync_read_u32<'a, F>(&'a mut self, motor_ids: &'a [u8], address: u16) -> Result<Vec<SyncData<u32>>, TransferError> {
		let mut result = Vec::with_capacity(motor_ids.len());
		let mut read_error = None;
		self.sync_read_u32_cb(motor_ids, address, |data| match data {
			Err(e) if read_error.is_none() => read_error = Some(e),
