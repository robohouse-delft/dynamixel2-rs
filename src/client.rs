...
use crate::{bus_types::StatusPacket, ReadError, TransferError, WriteError};
	...
	Buffer: AsRef<[u8]> + AsMut<[u8]>,
{
	...
	pub async fn read_status_response_timeout<'a>(
		&'a mut self,
		timeout: Duration,
	) -> Result<StatusPacket<'a>, ReadError<SerialPort::Error>> {
	...
	pub async fn read_status_response<'a>(
		...
		let message_size = crate::bus_types::StatusPacket::message_len(expected_parameters as usize) as u32;
		let timeout = crate::bus_types::message_transfer_time(message_size, self.bus.baud_rate) + Duration::from_millis(34);
