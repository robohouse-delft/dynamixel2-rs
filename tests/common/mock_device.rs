...
use dynamixel2::{instructions::instruction_id::STATUS, Device, Instructions, ReadError, SerialPort};
		...
		thread::spawn(move || {
			while !kill.load(Relaxed) {
				...
				match packet.instruction {
					Instructions::Ping => {
						let_assert!(Ok(_) = self.device.write_status(self.id, 0, 3, |buffer| {
                            buffer[0] = 10;
                            buffer[1] = 20;
                            buffer[2] = 46;
                            Ok(())
					...
					Instructions::Read { address, length } => {
						if let Some(data) = self.control_table.read(address, length) {
							let_assert!(Ok(()) = self.device.write_status(self.id, 0, length as usize, |buffer| {
					...
					Instructions::SyncWrite { address, length, parameters } => {
					...
					Instructions::BulkWrite { parameters } => {
						...
						let _ = self.control_table.write(address, &parameters[id_index + 5..id_index + 5 + length as usize]);
					}
	...
	fn handle_sync_bulk_read(&mut self, address: u16, length: u16, ids: Vec<u8>) {
		for next_id in ids.clone() {
			if next_id == self.id {
				...
				loop {
					use dynamixel2::Instruction;
					if Instant::now() > deadline {
						warn!("{} timed out waiting for {}", self.id, next_id);
						// skip waiting for this id, maybe it's not coming
						break;
					}
					trace!("{} waiting for packet from {}", self.id, next_id);
					// todo: this currently doesn't work due to status packets not being returned
					let packet = self.device.read(Duration::from_millis(10));
					if matches!(packet, Ok(Instruction { id, instruction: Instructions::Unknown { instruction: STATUS, .. }}) if id == next_id)
					{
						break;
					}
				}
