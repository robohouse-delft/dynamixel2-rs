...
use dynamixel2::instructions::{PingInstruction, PingStatus};
...
fn do_main() -> Result<(), ()> {
	...
	dynamixel2::instructions::write_instruction(&mut tty, &PingInstruction::new(motor_id))
		.map_err(|e| eprintln!("failed to send PING instruction: {}", e))?;
	let status : PingStatus = dynamixel2::instructions::read_status(&mut tty)
